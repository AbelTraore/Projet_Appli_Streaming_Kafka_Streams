...
class JSONDeserializerDtlCommandes extends Deserializer[DetailsCommande]{
  ...
  override def deserialize(topic: String, data: Array[Byte]): DetailsCommande = {
    ...
    try {
      ...
      val d = objetMapper.readValue(data, classOf[DetailsCommande])
      d
