...
class FilterTopologyTestScalaTest extends AnyFlatSpec {
  ...
  val filterProcessorTopology =  FilterProcessors
  val testDriver = new TopologyTestDriver(filterProcessorTopology.topologie(),props)
  ...
  "le nombre d'article de la facture" should("être égal à 15 :") in {
    ...
    val factureFactory  : ConsumerRecordFactory[String, Facture]  = new ConsumerRecordFactory("factureBinJSO", stringSes, factureSes)
